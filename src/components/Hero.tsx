...
export function Hero() {
  ...
  const prevSlide = useCallback(() => {
    if (!isTransitioning) {
      setIsTransitioning(true);
      setCurrentSlide((prev) => (prev - 1 + content.heroData.length) % content.heroData.length);
      setTimeout(() => setIsTransitioning(false), 500);
    }
  }, [isTransitioning]);
  ...
  return (
    ...
    >
      ...
      {content.heroData.map((slide, index) => (
        <div
          key={slide.id}
          className={`absolute inset-0 w-full h-full transform duration-500 ease-in-out ${
            index === currentSlide
              ? 'translate-x-0 z-10'
              : index < currentSlide
